...
class ResearchGroupInvitationIntegrationTest extends AbstractIntegrationTest {
    ...
    void shouldInviteExistingUserAndShowInvitationInTheirList() throws Exception {
        ...
        mockMvc.perform(post("/api/research-groups/" + group.getId() + "/invitations")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(invitationPayload("invitee@example.com", ResearchGroupMemberRole.ANNOTATOR, expiresAt)))
    ...
    void shouldInviteEmailWithoutAccountAndSendSignupEmail() throws Exception {
        ...
        mockMvc.perform(post("/api/research-groups/" + group.getId() + "/invitations")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(invitationPayload("new.researcher@example.com", ResearchGroupMemberRole.ADMIN, expiresAt)))
                .andExpect(status().isCreated())
    ...
    void shouldReturnConflictWhenInvitationAlreadyExists() throws Exception {
        ...
        mockMvc.perform(post("/api/research-groups/" + group.getId() + "/invitations")
                .session(ownerSession)
                .contentType(MediaType.APPLICATION_JSON)
                .content(invitationPayload("duplicate@example.com", ResearchGroupMemberRole.ANNOTATOR, expiresAt)))
    ...
    void shouldReturnConflictWhenInvitedEmailAlreadyBelongsToMember() throws Exception {
        ...
        mockMvc.perform(post("/api/research-groups/" + group.getId() + "/invitations")
                .session(ownerSession)
                .contentType(MediaType.APPLICATION_JSON)
                .content(invitationPayload("member@example.com", ResearchGroupMemberRole.ANNOTATOR, expiresAt)))
                .andExpect(status().isConflict());
    }
    ...
    void shouldReturnForbiddenWhenAnnotatorTriesToInvite() throws Exception {
        ...
        mockMvc.perform(post("/api/research-groups/" + group.getId() + "/invitations")
                .session(session)
                .contentType(MediaType.APPLICATION_JSON)
                .content(invitationPayload("target@example.com", ResearchGroupMemberRole.ANNOTATOR, expiresAt)))
    ...
    void shouldReturnServiceUnavailableWhenInvitationEmailFails() throws Exception {
        ...
        doThrow(new RuntimeException("smtp down"))
                .when(emailService)
                .sendResearchGroupInvitationToExistingUser(eq("invitee2@example.com"), eq("Failure Group"),
        ...
        mockMvc.perform(post("/api/research-groups/" + group.getId() + "/invitations")
                .session(ownerSession)
                .contentType(MediaType.APPLICATION_JSON)
                .content(invitationPayload("invitee2@example.com", ResearchGroupMemberRole.ANNOTATOR, expiresAt)))
