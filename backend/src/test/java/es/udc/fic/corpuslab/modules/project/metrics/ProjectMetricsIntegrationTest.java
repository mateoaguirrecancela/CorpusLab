...
@ActiveProfiles("test")
class ProjectMetricsIntegrationTest extends AbstractIntegrationTest {
    ...
    @Test
    void getProjectMetricsShouldReturnNotFoundWhenProjectIsNotAssigned() throws Exception {
        createUser("metrics.viewer@example.com");
        String session = loginAs("metrics.viewer@example.com");

        mockMvc.perform(get("/api/projects/{projectId}/metrics", 999L)
    ...
    @Test
    void getProjectMetricsShouldReturnUnauthorizedWithoutSession() throws Exception {
        mockMvc.perform(get("/api/projects/{projectId}/metrics", 999L))
