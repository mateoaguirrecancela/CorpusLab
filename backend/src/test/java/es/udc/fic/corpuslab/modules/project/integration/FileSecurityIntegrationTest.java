...
class FileSecurityIntegrationTest extends AbstractIntegrationTest {
    ...
    void setup() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/api/auth/login")
                ...
                .content(objectMapper.writeValueAsString(
                        UserLoginRequestTestBuilder.validRequest().withEmail(email).withPassword("password").build())))
    ...
    void shouldRejectPathTraversalInFilename() throws Exception {
        ...
        mockMvc.perform(multipart("/api/research-groups/{groupId}/projects/{projectId}/dataset", groupId, projectId)
                ...
                .andExpect(jsonPath("$.message").value("Invalid dataset upload: Invalid filename: potential path traversal"));
    }
    ...
    void shouldRejectForbiddenExtensions() throws Exception {
        ...
        mockMvc.perform(multipart("/api/research-groups/{groupId}/projects/{projectId}/dataset", groupId, projectId)
                ...
                .header("Authorization", "Bearer " + sessionToken))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value("Invalid dataset upload: Forbidden file type: exe"));
    }
    ...
    void shouldRejectSpoofedBinaryFilesUsingMagicBytes() throws Exception {
        ...
        mockMvc.perform(multipart("/api/research-groups/{groupId}/projects/{projectId}/dataset", groupId, projectId)
                ...
                .header("Authorization", "Bearer " + sessionToken))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value("Invalid dataset upload: Binary/Executable files are not allowed"));
    }
    ...
    private List<es.udc.fic.corpuslab.modules.project.shared.entities.DatasetItem> waitForDatasetItems()
