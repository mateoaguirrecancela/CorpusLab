...
class NotificationRepositoryTest {
    ...
    void findByRecipientUserIdOrderByCreatedAtDescShouldReturnLatestFirstWithLimit() {
        User recipient = userRepository.save(UserTestBuilder.validUser().withEmail("recipient@example.com").build());
        User actor = userRepository.save(UserTestBuilder.validUser().withEmail("actor@example.com").build());

        Notification oldest = saveNotification(recipient, actor, NotificationType.RESEARCH_GROUP_INVITATION_RECEIVED,
                Instant.parse("2026-04-04T08:00:00Z"), null);
        Notification middle = saveNotification(recipient, actor, NotificationType.RESEARCH_GROUP_INVITATION_ACCEPTED,
                Instant.parse("2026-04-04T09:00:00Z"), Instant.parse("2026-04-04T09:30:00Z"));
        Notification latest = saveNotification(recipient, actor, NotificationType.RESEARCH_GROUP_INVITATION_RECEIVED,
    ...
    void countByRecipientUserIdAndReadAtIsNullShouldOnlyCountUnread() {
        User recipient = userRepository.save(UserTestBuilder.validUser().withEmail("recipient2@example.com").build());
    ...
    void findByIdAndRecipientUserIdShouldRespectOwnership() {
        User recipient = userRepository.save(UserTestBuilder.validUser().withEmail("recipient3@example.com").build());
    ...
    void markAllAsReadByRecipientUserIdShouldOnlyUpdateUnreadForRecipient() {
        User recipient = userRepository.save(UserTestBuilder.validUser().withEmail("recipient5@example.com").build());
        User otherRecipient = userRepository
        ...
        Notification unreadMine = saveNotification(recipient, actor,
                NotificationType.RESEARCH_GROUP_INVITATION_RECEIVED,
                Instant.parse("2026-04-04T08:00:00Z"), null);
        Notification readMine = saveNotification(recipient, actor, NotificationType.RESEARCH_GROUP_INVITATION_ACCEPTED,
