...
class ProjectDatasetUtilsTest {
    ...
    @Test
    void parseCsvDatasetContentShouldHandleQuotedMultilineFields() {
        DatasetItem datasetItem = datasetItemWithId(
                202L,
                0,
                "multiline.csv",
                "text/csv",
                "text,label\n\"hello\nworld\",Greeting\n\"comma, kept\",Punctuation");

        ProjectDatasetUtils.CsvDatasetContent csvContent = ProjectDatasetUtils.parseCsvDatasetContent(datasetItem);

        assertThat(csvContent.headers()).containsExactly("text", "label");
        assertThat(csvContent.steps()).hasSize(2);
        assertThat(csvContent.steps().getFirst().rowValues())
                .containsEntry("text", "hello\nworld")
                .containsEntry("label", "Greeting");
        assertThat(csvContent.steps().get(1).rowValues())
                .containsEntry("text", "comma, kept")
                .containsEntry("label", "Punctuation");
    }
