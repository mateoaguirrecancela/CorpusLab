...
class ProjectParticipantServiceImplTest {
    ...
    void setUp() {
        projectParticipantService = new ProjectParticipantServiceImpl(
                ...
                notificationService, emailService, projectMetricsCacheService, entityReferenceService,
                new ProjectParticipantAssignmentPolicy(),
                new IaaGroupAssignmentPolicy(),
                "http://localhost:5173");
    }
    ...
    void assignParticipants_ShouldRemoveOldAndAddNewParticipants() {
        ...
        doAnswer(invocation -> {
            ProjectParticipant participant = invocation.getArgument(0);
            participant.setUser(toAdd);
            return null;
        }).when(entityReferenceService).attachUser(any(ProjectParticipant.class), eq(4L));
