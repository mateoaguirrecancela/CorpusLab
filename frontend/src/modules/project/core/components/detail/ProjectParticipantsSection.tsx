...
import { normalizeCompletionPercentage } from '@/modules/project/shared/utils/projectUtils';
...
export function ProjectParticipantsSection({ project }: ProjectParticipantsSectionProps) {
  ...
  const canOpenAnnotationWorkspace = project.setupCompleted && !project.archived;
                  ...
                  <TableRow className="hover:bg-transparent" key={participant.userId}>
                    ...
                    {project.canManageProject && (
                      <TableCell className="px-4 py-3 text-right">
                        {participant.role === 'PARTICIPANT' && canOpenAnnotationWorkspace ? (
                          <Link
