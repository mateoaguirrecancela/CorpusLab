...
export function useAnnotationTrends(): AnnotationTrendsState {
  ...
  const series = useMemo(() => {
    if (selectedGroupFilter === ALL_GROUPS_FILTER) {
      return allSeries;
    }

    return allSeries.filter((item) => item.dataKey === selectedGroupFilter);
  }, [allSeries, selectedGroupFilter]);
  ...
  useEffect(() => {
    if (
      ...
      setSelectedGroupFilter(ALL_GROUPS_FILTER);
    }
  }, [allSeries, selectedGroupFilter]);
